...
export const handler = async(event) => {
    ...
    return await middleware.Handler.main(event, async function({ token, body }){
        ...
        logger.info(`Recovering root user key.`);
        ...
        try { keys = new schema.Request.Keys.Recover(body.keys) }

        /// Return if body is malformed
        catch(e) { 
            logger.error(`Failed to construct body data. ${ e.stack }`);

            return new schema.Response.Keys.Recover({
                statusCode : 400,
        ...
        logger.info(`Recovery key detected. Proceeding with the recovery step.`)
