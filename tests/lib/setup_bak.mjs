...
export default class Setup {
    ...
    async getSampleKeys() {
        ...
        /// Generate a random IV value
        const recovery_iv = crypto.getRandomValues(new Uint8Array(12));

        /// Generate recovery secret
        const recovery_secret = await crypto.subtle.generateKey(
            {
                name: "AES-GCM",
                length: 256
            },
        ...
        /// Generate a random IV value
        const recovery_secret_iv = crypto.getRandomValues(new Uint8Array(12));
        ...
        const master_recovery_wrapped = Buffer.from(await crypto.subtle.encrypt(
            { name : "AES-GCM", iv: recovery_secret_iv },
            recovery_secret,   
            await crypto.subtle.exportKey("pkcs8", master.privateKey)
        )).toString("base64");

        /// Wrap recovery secret using derived value from recovery phrase
        const recovery_secret_wrapped = Buffer.from(await crypto.subtle.encrypt(
            { name : "AES-GCM", iv: recovery_iv },
            recovery_wk,   
            await crypto.subtle.exportKey("raw", recovery_secret)
        )).toString("base64");
                ...
                keys : {
                    rsa : {
                        ...
                        privateKey : {
                            ...
                            wrapper : `${ this.APP_ID_ROOT }.userKey.secret`
                        }
                    }
                }
            },
            ...
            recovery_key : {
                ...
                keys : {
                    master : {
                        ...
                        iv : Buffer.from(recovery_secret_iv).toString("base64"),
                        wrapper : "this.recoveryKey.aes"
                    },
                    secret : {
                        value : recovery_secret_wrapped,
                        iv : Buffer.from(recovery_iv).toString("base64"),
            ...
            root_key : {
                ...
                keys : {
                    ...
                    secret : {
                        ...
                        wrapper : `${ this.APP_ID_ROOT }.userKey.pbkdf2`
                    },
                    pbkdf2 : {
                        algorithm : {
                            name: 'PBKDF2',
